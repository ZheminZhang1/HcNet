...
class HcRaBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, mlp_ratio=4., drop=0., drop_path=0.,
        ...
        # ------------------------------0------------------------------------------
        self.Anisotropic_conv = nn.Conv2d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=False)
    ...
    def forward(self, x):
        ...
        center_elements = self.Anisotropic_conv.weight[:, :, 1, 1]
        center_elements = center_elements.unsqueeze(0).unsqueeze(-1)

        x_around_sum_0 = Anisotropic_x - x_conv_0 *center_elements

        center_weight = 2.0 * self.Anisotropic_conv.weight.sum(dim=(-2, -1))
        center_weight = center_weight.unsqueeze(0).unsqueeze(-1)

        second_Der_0 = x_around_sum_0 - x_conv_0 *center_weight
        # -----------------------
        input_Dependency_0 = self.avg_pool(x)
        alpha_0 = self.fc_0(input_Dependency_0)
        alpha_DeltaT_0 = self.sigmoid(alpha_0)
        # -----------------------
        x = shortcut_0 + alpha_DeltaT_0 * second_Der_0
        # -------------------------------------------------------------------------
...
class HcNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=[64, 128, 320, 512], depths=[ 5, 5, 14, 5 ], mlp_ratio=4.,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim[i_layer]),
                               ...
                               downsample_outdim=int(embed_dim[i_layer +1]) if
